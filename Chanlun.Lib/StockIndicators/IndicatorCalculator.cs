...
{
    public static float[] Calculate(int nCount, float[] pOpen, float[] pClose, float[] pKey)
    {
        var key = pKey[0];
        ...
        var units = kLines.SelectMany(p => p.CombinedUnits).ToList();

        foreach (var unit in units)
        {
            unit.Open = pOpen[unit.Idx];
            unit.Close = pClose[unit.Idx];
        }
    ...
    private static float CalculateBiMetric1(Bi.Bi bi)
    {
        double energy = 0.0;
    ...
    private static float CalculateBiMetric(Bi.Bi bi)
    {
        var units = new List<KLineUnit>();
        var curLine = bi.StartChanKLine;
        while (curLine != null && curLine.Idx <= bi.EndChanKLine.Idx)
        ...
        return (float)(macdArea * bollExtension);
    }
