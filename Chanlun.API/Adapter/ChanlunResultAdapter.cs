        ...
        if (indicatorResult.ChanCalculateResult.UnitList?.Count > 0)
        {
            ...
            for (int i = 0; i < indicatorResult.ChanCalculateResult.UnitList.Count; i++)
            {
                ...
                response.Bars.Add(new TvKlineBar()
                {
                    ...
                    KdjD = item.Kdj.D,
                    KdjJ = item.Kdj.J,
                    KdjSignal = item.Kdj.Signal,
                    KdjBullish = item.Kdj.IsBullish,
                    KdjGoldenCross = item.Kdj.IsGoldenCross,
                    KdjDeathCross = item.Kdj.IsDeathCross,
                    KdjBottomDivergence = item.Kdj.IsBottomDivergence,
                    KdjTopDivergence = item.Kdj.IsTopDivergence,
                    Rsi6 = item.Rsi.Rsi6,
